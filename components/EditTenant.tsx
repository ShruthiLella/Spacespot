...
type TenantFormData = {
  ...
  abn: string;
  acn: string;
  verificationStatus: TenantStatus;
...
const tenantDetailsLabelStyle: React.CSSProperties = {
  fontSize: '10px',
  color: '#243447',
  marginBottom: '6px',
  display: 'block',
...
export default function EditTenant() {
  ...
  const { tenantId = 'A' } = useParams();
  const navState = location.state as TenantNavState | undefined;
  const selectedTenant = navState?.tenant;

  const initialForm = useMemo<TenantFormData>(() => ({
    tenantBrandName: selectedTenant?.brandName || 'ABC Tenant',
    brandCategory: selectedTenant?.category || 'Retail',
    ownerName: selectedTenant?.ownerName || 'John Phillip',
    referenceName: 'Mark Taylor',
    tenantLevel: selectedTenant?.level || 'Premium',
    email: selectedTenant?.email || 'admin@abctenant.com',
    primaryPhone: selectedTenant?.phone || '412356789',
    secondaryPhone: '412356781',
    businessWebsite: 'www.abctenant.com',
    ...
    abn: '3456789012',
    acn: '2345678901',
    verificationStatus: selectedTenant?.status || 'Verified',
  ...
  const updateField = (key: keyof TenantFormData, value: string) => {
    setForm((prev) => ({ ...prev, [key]: value }));
  };

  const completion = useMemo(() => {
    const values = [
      form.tenantBrandName,
      form.brandCategory,
      form.ownerName,
      form.tenantLevel,
      form.email,
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '0' }}>
              ...
              <div>
                ...
                <div style={{ fontSize: '8px', color: '#8fa0b2', marginTop: '1px' }}>
                  Update tenant information and payment details
                </div>
              </div>
            </div>
          ...
          >
            <div style={{ ...sectionCard, padding: '8px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '7px', marginBottom: '8px' }}>
                ...
                <span style={{ fontSize: '10px', fontWeight: 600, color: 'var(--spacespot-navy-primary)' }}>Tenant Summary</span>
              </div>
              ...
              <div style={{ backgroundColor: '#d9edf0', borderRadius: '6px', padding: '8px', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '7px' }}>
                ...
                <div>
                  <div style={{ fontSize: '9px', color: 'var(--spacespot-navy-primary)', fontWeight: 600 }}>{form.tenantBrandName}</div>
