      ...
      <div style={{ maxWidth: '1060px', margin: '0 auto' }}>
              ...
              {['All', 'Verified', 'Pending', 'Initiated'].map((item) => {
                ...
                return (
                  <button
                    key={item}
                    type="button"
                    onClick={() => setFilter(item as 'All' | CustomerStatus)}
                    style={{
                      border: 'none',
                      height: '24px',
                      borderRadius: '6px',
        ...
        <div style={{ ...cardBase, padding: '10px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
            <div style={{ fontSize: '12px', fontWeight: 700, color: 'var(--spacespot-navy-primary)' }}>Customer List ({filteredCustomers.length})</div>
            <button
              type="button"
              style={{
                border: 'none',
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '900px' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid #e6edf4' }}>
                  {['CUSTOMER', 'CONTACT', 'CATEGORY', 'LEVEL', 'STATUS', 'JOINED', 'ACTIONS'].map((head) => (
                    <th
                      key={head}
                      style={{
              ...
              <tbody>
                {filteredCustomers.map((customer) => (
                  <tr key={customer.email} style={{ borderBottom: '1px solid #eef3f8' }}>
                    <td style={{ padding: '9px 6px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        ...
                        <div>
                          <div style={{ fontSize: '10px', color: 'var(--spacespot-navy-primary)', fontWeight: 600 }}>{customer.brandName}</div>
                          <div style={{ fontSize: '8px', color: '#95a4b5' }}>{customer.ownerName}</div>
                        </div>
                      </div>
                    </td>
                    <td style={{ padding: '9px 6px' }}>
                      <div style={{ fontSize: '8px', color: '#95a4b5', display: 'grid', gap: '2px' }}>
                        <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}><Mail size={8} /> {customer.email}</span>
                        <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}><Phone size={8} /> {customer.phone}</span>
                      </div>
                    </td>
                    <td style={{ padding: '9px 6px' }}>
                      <span style={{ fontSize: '9px', color: '#3c536d', display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
                        <Building2 size={8} color="var(--spacespot-cyan-primary)" /> {customer.category}
                      </span>
                    </td>
                    <td style={{ padding: '9px 6px' }}>
                      <span style={{ ...levelStyle(customer.level), fontSize: '7px', borderRadius: '4px', padding: '1px 4px' }}>
                        {customer.level}
                      </span>
                    </td>
                    <td style={{ padding: '9px 6px' }}>
                      <span style={{ ...chipStyle(customer.status), fontSize: '8px', borderRadius: '999px', padding: '2px 8px', display: 'inline-flex', alignItems: 'center', gap: '3px', fontWeight: 600 }}>
                        <CircleCheck size={8} /> {customer.status}
                      </span>
                    </td>
                    <td style={{ padding: '9px 6px', fontSize: '8px', color: '#7e8ea0', lineHeight: 1.2, whiteSpace: 'pre-line' }}>
                      {customer.joinedDate.replace(', ', ',\n')}
                    </td>
                    <td style={{ padding: '9px 6px' }}>
                      <div style={{ display: 'flex', justifyContent: 'center', gap: '8px' }}>
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            navigate(`/manage/edit-customer/${customer.id}`, {
                              state: { customer },
                            })
                          }
          ...
          {filteredCustomers.length === 0 && (
            <div style={{ padding: '24px 0', display: 'grid', placeItems: 'center', color: '#92a2b4' }}>
              ...
              <div style={{ marginTop: '8px', fontSize: '10px' }}>No customers match the selected filters.</div>
            </div>
          )}
        </div>
      </div>
